...
int gl_test_mul_lights()
{
	...
	// ------------------------------------
	Shader lightingShader("shader/8_mul_lights.vs", "shader/8_mul_lights.fs");
	Shader lampShader("shader/4_lamp.vs", "shader/4_lamp.fs");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		{
			char buff[50];
			char prefixFormat[] = "pointLights[%d]";
			sprintf(buff, prefixFormat, i);
			std::string prefix(buff);
			lightingShader.setVec3(prefix + ".position", pointLightPositions[i]);
			lightingShader.setVec3(prefix + ".ambient", 0.2f, 0.2f, 0.2f);
			lightingShader.setVec3(prefix + ".diffuse", 0.5f, 0.5f, 0.5f);
